...
**Oportunidades no mês** (por vendedora e da equipe): tarefas vinculadas a oportunidades com `data_prevista` no mês, atribuídas pelo `responsavel` da tarefa (quase nenhuma oportunidade é criada pela própria vendedora — vêm do SDR/importação; o vínculo é a tarefa, mesma ideia do "Oport. em Andamento" do Analítico). Mostra oportunidades trabalhadas (distintas), tarefas **feitas** (Realizada) e **não feitas** (Pendente/Em Execução), com as **atrasadas** (data prevista já passou) destacadas e a lista das não feitas. Canceladas não contam. Carregado à parte das vendas, então funciona mesmo sem a migração 0032.

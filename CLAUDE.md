...
**Grupo "Tarefas e Oportunidades"** (por vendedora e da equipe; carregado à parte das vendas, não depende das migrações): tarefas pelo `responsavel` (mesmo critério do "Minha fila"), de qualquer vínculo, no mês da `data_prevista` (sem data, no mês de criação). **Nova Tarefa** = criadas no mês; **Hoje / Tarefas Futuras / Atrasadas** = `classificarPrazo`, a mesma função das pills do FunilBoard (relativas a hoje — num mês passado, pendência vira Atrasada), com lista das atrasadas; **Concluídas** = só Realizada (Cancelada não conta em nada). **Oportunidades** = registros da tabela `oportunidades` (não tarefas) com atividade no mês (criadas, movimentadas ou com tarefa no mês), atribuídos pela regra compartilhada `src/lib/oportunidades/atribuicao.ts` (tarefa aberta mais recente → última Realizada → criador — a mesma do "Oport. em Andamento" do Analítico, que passou a usar esse helper); **Oport. Concluídas** = das atribuídas, as que viraram GANHO no mês (data = `ultima_movimentacao`). Zerado enquanto ninguém usa "Converter em Orçamento" — é uso real, não bug.

**Grupo "Orçamentos e Pedidos"**: **Orçamentos** = propostas (diretos + normais criados no mês). **Orçamentos Cotados / Pedidos Aprovados / Efetuados / Entregues** = pedidos que **chegaram** a essa etapa ou a uma posterior, com a data da primeira chegada no mês — RPC `fn_pedidos_marcos` (migração 0033; `fn_pedidos_efetuados` passou a derivar dela). Nunca o status atual (o funil não encolhe). Atribuição por `criado_por` (quem vendeu) — diferente do "Entregues" do Analítico, que é por `movido_por` (quem operacionalizou), de propósito. Sem a 0033 rodada, a tela cai pra `fn_pedidos_efetuados` e mostra "—" em Cotados/Aprovados/Entregues.

**Impressão:** botão "🖨️ Imprimir" ao lado do seletor de mês. Versão dedicada `ProdutividadeImpressao.tsx` (A4 paisagem, papel branco): cabeçalho com logo da empresa ativa, "Produtividade Comercial", empresa e mês; seções Resultado da empresa (meta global, faturado, % atingida, não faturado), Metas por vendedora (meta pessoal, faturado, % pessoal, % da global), Tarefas e Oportunidades e Orçamentos e Pedidos (linha EQUIPE só pro gestor — o comercial imprime só a própria linha, mesmo recorte da tela); rodapé com data/hora. Mesma base do Analítico: `src/components/impressao/FolhaImpressao.tsx` (`FolhaImpressao` + `TabelaImpressao`) e o bloco CSS "Folha de impressão" do `globals.css` (`.folha-impressao-*`, `@page folha-a4`) — o `AnaliticoPorFuncionarioImpressao` virou um wrapper dela. Cabeçalho e rodapé ficam numa tabela-moldura (thead/tfoot repetem em toda página). A Produtividade usa a variante `relatorio` da folha (`FolhaImpressao variante="relatorio"`, classe `.folha-impressao--relatorio`): fonte e espaçamentos maiores (base 14pt, números 18pt, indicadores 22pt) e diagramação em grade 2×2 pra aproveitar a paisagem — em cima, resultado da empresa (indicadores 2×2) | metas por vendedora; embaixo, os dois grupos de contadores lado a lado, **transpostos** (um contador por linha, uma vendedora por coluna). Com mais de 4 vendedoras, os grupos voltam a uma linha por vendedora, empilhados em largura total. Cada faixa da grade muda de página inteira; a fonte nunca diminui. O Analítico continua na variante `compacta` (até 13 colunas).

**Migração 0033 — correção de base da auditoria:** os gatilhos declarados na 0004 (`trg_oportunidade_movimentada`, `trg_audit_oportunidades`) e na 0009 (`trg_audit_clientes`) nunca existiram no banco (tabelas criadas pelo bloco SQL da Fase 31, só `create table` + RLS; zero linhas em `audit_log` e oportunidades PERDIDAS com `movido_por` nulo). A 0033 os recria + cria `trg_audit_tarefas`, e termina com uma consulta ao `pg_trigger` pra conferência imediata. `chamadas` fica de fora (sync a cada 15 min encheria o log). Dados anteriores à 0033 seguem com aproximação (data prevista, situação atual, `ultima_movimentacao`); só dali pra frente as datas de tarefas/oportunidades são exatas.

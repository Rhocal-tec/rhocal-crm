...
// Tarefa Realizada sempre cai em Concluídas, independente da data. Tarefa
// sem data_prevista (campo opcional) entra em Futuras — não é uma pendência
// com prazo vencido. Comparação por string 'YYYY-MM-DD' (sem passar por
// Date) evita o off-by-one de fuso horário já documentado em
// formatarDataSomente (lib/kanban/formatacao.ts).
export function classificarPrazo(
  tarefa: Pick<Tarefa, 'situacao' | 'data_prevista'>,
): PrazoBucket {
  if (tarefa.situacao === 'Realizada') return 'CONCLUIDAS'
